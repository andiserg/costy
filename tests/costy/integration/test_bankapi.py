...
@pytest.mark.asyncio()
async def test_create_bankapi(costy_app, db_session, db_tables, clean_up_db):
...
@pytest.mark.asyncio()
async def test_delete_bankapi(costy_app, db_session, db_tables, clean_up_db, bankapi_gateway):
    bankapi = BankAPI(
        user_id=1,
        name="monobank",
        access_data={"X-Token": "aboba"},
