...
from dishka import Provider, provide, Scope, from_context, decorate
...
class IdDIProvider(Provider):
    @provide
    async def get_id_provider(self, r: Request, web_session: AsyncClient, settings: AuthSettings,
                              user_gateway: UserGateway) -> IDP:
        token_processor = JwtTokenProcessor("RS256", settings.audience, settings.issuer)
        jwsk_provider = KeySetProvider(settings.jwks_uri, web_session, timedelta(days=1))
        ...
        id_provider.token = token  # type: ignore[attr-defined]
...
class DIProvider(Provider):
    ...
    async def get_session(self, session_maker: async_sessionmaker[AS]) -> AsyncIterable[AS]:
    ...
    @provide(scope=Scope.REQUEST)
    async def get_user_gateway(self, session: AS, tables: dict[str, Table]) -> UserGateway:
    ...
    ) -> dict[str, BankGateway]:
        return {
            "monobank": MonobankGateway(web_session, banks_conf),
        }
    ...
    @provide(scope=Scope.REQUEST)
    async def get_operation_dependencies(self, gateway: OG, session: AS) -> tuple[OG, AS]:
        return gateway, session


    @provide(scope=Scope.REQUEST)
    async def get_category_dependencies(self, gateway: CG, session: AS) -> tuple[CG, AS]:
        return gateway, session
    ...
    @provide
    async def get_create_user(self, user_gateway: UG, auth_gateway: AG, session: AS) -> CreateUser:
        return CreateUser(UserService(), user_gateway, auth_gateway, session)


    @provide
    async def get_create_operation(self, depends: tuple[OG, AS], idp: IDP) -> CreateOperation:
        return CreateOperation(OperationService(), depends[0], idp, depends[1])


    @provide
    async def get_read_list_operation(self, depends: tuple[OG, AS], idp: IDP) -> ReadListOperation:
        return ReadListOperation(OperationService(), depends[0], idp, depends[1])


    @provide
    async def get_delete_operation(self, depends: tuple[OG, AS], idp: IDP) -> DeleteOperation:
        return DeleteOperation(AccessService(), depends[0], idp, depends[1])


    @provide
    async def get_update_operation(self, depends: tuple[OG, AS], idp: IDP) -> UpdateOperation:
        return UpdateOperation(OperationService(), AccessService(), depends[0], idp, depends[1])


    @provide
    async def get_create_category(self, depends: tuple[CG, AS], idp: IDP) -> CreateCategory:
        return CreateCategory(CategoryService(), depends[0], idp, depends[1])


    @provide
    async def get_delete_category(self, depends: tuple[CG, AS], idp: IDP) -> DeleteCategory:
        return DeleteCategory(AccessService(), depends[0], idp, depends[1])


    @provide
    async def get_update_category(self, depends: tuple[CG, AS], idp: IDP) -> UpdateCategory:
        return UpdateCategory(CategoryService(), AccessService(), depends[0], idp, depends[1])


    @provide
    async def get_read_available_categories(self, depends: tuple[CG, AS], idp: IDP) -> ReadAvailableCategories:
        return ReadAvailableCategories(CategoryService(), depends[0], idp, depends[1])


    @provide
    async def get_create_bankapi(self, depends: tuple[BG, AS], idp: IDP) -> CreateBankAPI:
        return CreateBankAPI(BankAPIService(), depends[0], idp, depends[1])


    @provide
    async def get_delete_bankapi(self, depends: tuple[BG, AS], idp: IDP) -> DeleteBankAPI:
        return DeleteBankAPI(AccessService(), depends[0], idp, depends[1])


    @provide
    async def get_read_bankapi_list(self, gateway: BankAPIGateway, idp: IdProvider) -> ReadBankapiList:
